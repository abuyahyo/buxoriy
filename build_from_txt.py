...
# Манбада `*` белгиси қўйилган, лекин изоҳ матни ёзилмаган сўзлар учун
# китобнинг ўз луғатидан (бошқа ҳадислардаги изоҳлардан) олинган изоҳлар.
FOOTNOTE_ADD = {
    7: '«Сижол» – аслида қудуқдан навбатма-навбат сув олишга айтилади; жангда гоҳ ютиб, гоҳ ютқазиш шунга ўхшатилган.',
    1532: '«Батҳо» – водийдаги майда тошли кенг ўзан, сув йўли. Зулҳулайфадаги шундай жой ҳам Батҳо дейилади.',
    2359: '«Ҳарра» – тошлари қора бўлган тошлоқ жой; Мадинанинг ғарбидаги шундай жой.',
    2495: '«Шуфъа» – имтиёзли сотиб олиш ҳуқуқи (шерик ўз ҳиссасини сотса, биринчи навбатда шериги ҳақдор).',
    5931: '«Вашм» – танага игна билан расм/сўз ўйиб нақш солиш, татуировка.',
}


def final_clean(s):
    s = re.sub(r'\s*\[\d+\]', '', s)
    # сатр бошидаги "*Сўз" footnote белгисидан кейин бўш жой қўйиш ("* Сўз")
    s = re.sub(r'(^|\n)\*(?=[^\s*])', r'\1* ', s)
    s = re.sub(r'[ \t]{2,}', ' ', s)
    s = re.sub(r' +([,.;:!?»])', r'\1', s)
    return s.strip()
...
for b in books:
    ...
    for bo in b['boblar']:
        ...
        for h in bo['hadislar']:
            if h['id'] in FOOTNOTE_ADD:
                h['matn'] = h['matn'].rstrip() + '\n\n* ' + FOOTNOTE_ADD[h['id']]
